...
| endGame | Завершение игры и обработка результатов |
...
### 5.1. endGame
Завершение игры и обработка результатов.

**Параметры**
```
{
    token: string; - токен пользователя для идентификации
    score: number; - набранные очки пользователем
    death: number; - количество смертей пользователя
}
```
**Успешный ответ**
```
